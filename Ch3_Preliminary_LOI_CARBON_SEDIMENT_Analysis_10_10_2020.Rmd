...
df=as.data.frame(LOI_QAQC[c(1:13,32)])
...
LOI_QAQC=LOI%>%
 ...
 dplyr::mutate(Frac_Shell=(Shell_gDW)/Sample_Weight_gDW,
  ...
  dplyr::mutate(Sediment_Depth_cm=(Slice_Number*2)*Compaction_Ratio,
...
Grainsize_QAQC=Grainsize_QAQC%>%
  ...
  dplyr::mutate(Slice_Height_Corrected_cm=(Slice_Height_cm*Compaction_Ratio),Slice_Area_cm2=pi*(2.54^2),Slice_Volume_cm3=Slice_Area_cm2*Slice_Height_Corrected_cm, Sediment_Depth_cm=(Slice_Number*2)*Compaction_Ratio)%>%
...
LOI_QAQC_By_Site_Slice=LOI_QAQC%>%
  dplyr::filter(Sediment_Depth_cm<=40)%>%
  dplyr::group_by(Site,Factor,Quadrat,Bagged_Slice_Number,Slice_Number,Slice_Height_Corrected_cm,Sediment_Depth_Group)%>%
  dplyr::summarize(Sediment_Depth_cm=unique(round(Sediment_Depth_cm,2)),OLOI=mean(OLOI,na.rm=TRUE),ILOI=mean(ILOI,na.rm=TRUE),Organic_Content_Density_g_per_ml=mean(Organic_Content_Density_g_per_ml,na.rm=TRUE),Inorganic_Content_Density_g_per_ml=mean(Inorganic_Content_Density_g_per_ml,na.rm=TRUE),Percent_Carbon=mean(Percent_Carbon,na.rm=TRUE),Rhizome_Density_g_per_ml=mean(Rhizome_Density_g_per_ml,na.rm=TRUE),Shell_Density_g_per_ml=mean(Shell_Density_g_per_ml,na.rm=TRUE),Sediment_Density_g_per_ml=mean(Sediment_Density_g_per_ml,na.rm=TRUE),Sample_Density_g_per_ml=mean(Sample_Density_g_per_ml,na.rm=TRUE))%>%
  ungroup()%>%
...
#testing for relationship with slice depth 
shell_depth_lm=lm(Shell_Density_g_per_ml ~  Factor + Sediment_Depth_Factor, data=LOI_QAQC_By_Site_Slice)
...
LOI_QAQC_By_Site_Slice%>%
  ...
  mutate(Sediment_Depth_cm=as.integer(Sediment_Depth_cm))%>%
              ggplot(aes(y=(mean),x=-Sediment_Depth_cm,color=Factor,fill=Factor))+geom_point() + geom_smooth(method="lm",) + ylab("OC Density (g/cm3)") + xlab("Sediment Depth (cm)") + ggtitle("Organic Carbon Density by Sediment Depth ")+coord_flip()+facet_wrap(~Sediment_Depth_Factor, scale="free_y")


OCD_deep_lm=lm(Organic_Carbon_Density_g_per_ml ~ Factor + Sediment_Depth_cm + Sediment_Depth_Factor, data=LOI_QAQC_By_Site_Slice)
summary(OCD_deep_lm)
...
LOI_QAQC_By_Site_Slice%>%
  ...
  mutate(Sediment_Depth_cm=as.integer(Sediment_Depth_cm))%>%
              ggplot(aes(y=(mean),x=-Sediment_Depth_cm,color=Factor,fill=Factor))+geom_point() + geom_smooth(method="lm",) + ylab("IOC Density (g/cm3)") + xlab("Sediment Depth (cm)") + ggtitle("Inorganic Carbon Density by Sediment Depth ")+coord_flip()+facet_wrap(~Sediment_Depth_Factor, scale="free_y")
...
LOI_QAQC_Quadrat_Level=LOI_QAQC_By_Site_Slice%>%
  ...
  dplyr::mutate(Stock_Scaling_Factor=ifelse(Sediment_Depth_Factor=="shallow",15,25),
         ...
         Rhizome_g_per_sqm=Rhizome_accum_per_cm*(1/0.0001)*14,
...
LOI_QAQC_Site_Level=LOI_QAQC_Quadrat_Level%>%
...
#this is as far as I got on March 25 2021.
...
BlueCarbon_QAQC=join(BlueCarbon_QAQC,Meadow_Structure[c(4:5)],by=c("Site"),match="all",type="inner")
...
BlueCarbon_QAQC=BlueCarbon_QAQC[c(1:43)]%>%
...
Current.Velocity.Max=raster("C:/Users/jennm/Dropbox/GITHUB/Disseratation/Large_Project_DataSets/BioOracle/data_output/Present.Benthic.Mean.Depth.Current.Velocity.Max.tif")
names(Current.Velocity.Max)<-c("Current.Velocity.Max")
# 
...
Temperature.Mean.pts=raster::extract(x=Temperature.Mean,y=BlueCarbon_QAQC)
PPM.pts=raster::extract(x=Primary.productivity.Max,y=BlueCarbon_QAQC)
CV.pts=raster::extract(x=Current.Velocity.Max,y=BlueCarbon_QAQC)
...
CV.pts=as.data.frame(CV.pts)

BlueCarbon_QAQC=as.data.frame(cbind(BlueCarbon_QAQC,FWCC_Seagrass_Extent,Temperature.Mean.pts,PPM.pts,CV.pts))
names(BlueCarbon_QAQC)[45:48]<-c("FWCC_Seagrass_Extent","Temperature.Mean","Primary.productivity.Max","Current.Velocity.Max")
# 
summary(BlueCarbon_QAQC)
# names(BlueCarbon_QAQC)
# 
