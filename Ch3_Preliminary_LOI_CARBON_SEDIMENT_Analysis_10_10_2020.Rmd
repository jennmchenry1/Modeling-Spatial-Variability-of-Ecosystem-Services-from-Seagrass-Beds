...
#LOI Data
LOI=read.csv("data/Carbon/LOI_Data_version_03_03_2021.csv")
head(LOI)
names(LOI)
...
write.csv(LOI_QAQC,file="data_output/LOI_Data_version_QAQC_03_03_2020.csv")

#Grain size analysis 
Grainsize=read.csv("data/Carbon/Grainsize_Data_version_03_03_2021.csv")
...
write.csv(Grainsize_QAQC,file="data_output/Grainsize_Data_version_QAQC_03_03_2021.csv")
...
Grainsize_QAQC_long=Grainsize_QAQC%>%
  dplyr::select(1:2,9:20)%>%
  gather(key="Grainsize","Percent",9:14)
...
LOI_QAQC_By_Site_Slice=LOI_QAQC%>%
  ...
  dplyr::mutate(Organic_Carbon_Density_g_per_ml=(Percent_Carbon/100) * Organic_Density_g_per_ml,
...
#OM Density
ggplot(LOI_QAQC_By_Site_Slice,aes(y=(Organic_Density_g_per_ml),fill=Factor),na.rm=TRUE)+geom_boxplot() + ylab("Organic Density of Slice") + ylab("") +facet_wrap(~Sediment_Depth_Group,nrow=2)+ ggtitle("Organic Density by Sediment Depth Bin (g/ ml)")
...
#Inorganic Density
ggplot(LOI_QAQC_By_Site_Slice,aes(y=(Inorganic_Density_g_per_ml),fill=Factor),na.rm=TRUE)+geom_boxplot() + ylab("Carbonate Density of Slice") + xlab("") +facet_wrap(~Sediment_Depth_Group,nrow=2)+ ggtitle("Carbonate Density by Sediment Depth Bin (g/ ml)")
...
LOI_QAQC_By_Site=LOI_QAQC_By_Site_Slice%>%
  mutate(Organic_accum_per_cm=Organic_Density_g_per_ml*Slice_Height_Corrected_cm,
  Inorganic_accum_per_cm=Inorganic_Density_g_per_ml*Slice_Height_Corrected_cm,
  ...
  dplyr::summarize(OLOI=mean(OLOI,na.rm=TRUE),
                   ...
                   Inorganic_Carbon_accum_per_cm=mean(Inorganic_Carbon_accum_per_cm,na.rm=TRUE))%>%
  dplyr::mutate(OM_g_per_sqm=Organic_accum_per_cm*50*10000,
         IOM_g_per_sqm=Inorganic_accum_per_cm*50*10000,
         Shell_g_per_sqm=Shell_accum_per_cm*50*10000,
         Rhizome_g_per_sqm=Rhizome_accum_per_cm*10*10000,
         OC_Stock_g_per_sqm=Organic_Carbon_accum_per_cm*50*10000,
         IOC_Stock_g_per_sqm=Inorganic_Carbon_accum_per_cm*50*10000,
...
write.csv(seagrass_survey_AND_Clippings_By_Site,"data_output/Seagrass_Clippings_QAQC_Site_03_03_2021.csv")
...
ggplot(BlueCarbon_QAQC,aes(x=Seagrass_Spp,y=Organic_Carbon_Stock_g_per_sqm))+ geom_point() +geom_smooth(method="gam")
# 

ggplot(BlueCarbon_QAQC,aes(x=Thalassia.testudinum,y=Organic_Carbon_Stock_g_per_sqm))+ geom_point()+geom_smooth(method="gam")
...
ggplot(BlueCarbon_QAQC,aes(x=Syringodium.filiforme,y=Organic_Carbon_Stock_g_per_sqm))+ geom_point()+geom_smooth(method="gam")

BlueCarbon_QAQC%>%
  mutate(SY_PA=as.factor(SY_PA))%>%
  dplyr::filter(Factor=="Seagrass")%>%
  ggplot(aes(y=Organic_Carbon_Stock_g_per_sqm,x=SY_PA,fill=SY_PA)) + geom_boxplot()

ggplot(BlueCarbon_QAQC,aes(x=Halodule.wrightii,y=Organic_Carbon_Stock_g_per_sqm))+ geom_point()+geom_smooth(method="gam")


BlueCarbon_QAQC%>%
  mutate(HA_PA=as.factor(HA_PA))%>%
  dplyr::filter(Factor=="Seagrass")%>%
  ggplot(aes(y=Organic_Carbon_Stock_g_per_sqm,x=HA_PA,fill=HA_PA)) + geom_boxplot()
...
ggplot(BlueCarbon_QAQC,aes(x=Percent_Sands,y=Organic_Carbon_Stock_g_per_sqm))+ geom_point()+geom_smooth(method="gam")
